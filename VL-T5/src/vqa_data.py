...
import sys
sys.path.append('/home/ubuntu/VL-T5/VL-T5')
...
# coco_feature_dir = '/home/ubuntu/VL-T5/features'
coco_feature_dir = project_dir.joinpath('features')
...
class VQAFineTuneDataset(Dataset):
    def __init__(self, split='train', raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train'):
        ...
        self.sources = ['TextVQA_0.5.1_train' , 'TextVQA_0.5.1_val']
        if self.verbose:
        ...
        for source in self.sources:
            ...
            with open(data_info_path) as f:
                _data_info_dicts = json.load(f)['data'][:10]
                _clean_data_info_dicts = []
                for _d in _data_info_dicts:
                    ...
                    #     _d['source'] = source
                    URL = _d["flickr_original_url"]
                    if source == 'TextVQA_0.5.1_train' :
                        try:
                            # self.image_preprocess(URL)
                            _ = urllib.request.urlopen(URL).getcode()
                            self.img_ids_to_source[_d['image_id']] = 'train2014'
                            _clean_data_info_dicts.append(_d)
                            # print('Success: ', URL, '\nIMAGE ID: ', _d['image_id'])
                        except:
                            # print('Skipping: ', URL, '\nIMAGE ID: ',  _d['image_id'])
                            continue
                    else:
                        try:
                            # self.image_preprocess(URL)
                            _ = urllib.request.urlopen(URL).getcode()
                            self.img_ids_to_source[_d['image_id']] = 'val2014'
                            _clean_data_info_dicts.append(_d)
                            # print('Success: ', URL, '\nIMAGE ID: ',  _d['image_id'])
                        except:
                            # print('Skipping: ', URL, '\nIMAGE ID: ', _d['image_id'])
                ...
                data_info_dicts.extend(_clean_data_info_dicts)
            if self.verbose:
                print(f"Loaded {len(_clean_data_info_dicts)} data from", source)
    ...
    def __getitem__(self, idx):
        ...
        if self.args.use_vision:
            ...
            try:
                ...
                print(f"Error in prediction for {img_id}: {str(ex.args)}")

            # boxes.clamp_(min=0.0, max=1.0)
            out_dict['boxes'] = out_dict.get("normalized_boxes")
            out_dict['vis_feats'] = out_dict.get("roi_features")
...
def get_loader(args, split='karpathy_train', mode='train',
               batch_size=64, workers=4, distributed=False, gpu=0, topk=-1):
...
class VQADataset:
    ...
    def create_anslabel_dict(self, train2014_data, val2014_data):
        ans2label = {}
        ans2label[''] = 0
        index = 1

        for data_item in train2014_data['data']:
            majority_answer = self.most_frequent(data_item['answers'])
            if majority_answer not in ans2label:
                ans2label[majority_answer] = index
                index += 1

        for data_item in val2014_data['data']:
            majority_answer = self.most_frequent(data_item['answers'])
            if majority_answer not in ans2label:
                ans2label[majority_answer] = index
                index += 1
    ...
    def __init__(self, splits: str, verbose=True):
        ...
        with open(dataset_dir.joinpath(f'vqa/TextVQA_0.5.1_train.json')) as f:
            train2014_data = json.load(f)
        # with open(dataset_dir.joinpath(f'vqa/v2_mscoco_val2014_annotations.json')) as f:
        with open(dataset_dir.joinpath(f'vqa/TextVQA_0.5.1_val.json')) as f:
            val2014_data = json.load(f)
        train2014_id2datum = {}
        for datum in train2014_data['data']:
            qid = datum['question_id']
            train2014_id2datum[qid] = datum
        val2014_id2datum = {}
        for datum in val2014_data['data']:
            qid = datum['question_id']
        ...
        # Loading datasets
        self.data = []
        files = ['TextVQA_0.5.1_train.json', 'TextVQA_0.5.1_val.json']
        for idx, split in enumerate(self.splits):
            print('Loading split = ', split)
            self.data.extend(
                json.load(open(vqa_dir.joinpath(files[idx])))['data'])
        ...
        self.ans2label, self.label2ans = self.create_anslabel_dict(train2014_data, val2014_data)
        assert len(self.ans2label) == len(self.label2ans)

        if verbose:
            print('First 10 answers in train, val: ', list(self.ans2label.items())[:10])
